...
export class GeneralService {
  ...
  async getContracts(
    ...
    if (type === 'deliveryman') {
      const [deliveryPersons, total] = await this.deliveryPersonRepository
        ...
        .leftJoinAndSelect('dp.DeliveryPersonDocuments', 'doc', 'doc.contact = :contact', {
          contact: true,
        })
        .where(q ? `(client.last_name ILIKE :q OR client.first_name ILIKE :q)` : '1=1', {
          q: `%${q}%`,
        })
      ...
      for (const dp of deliveryPersons) {
        ...
        const profilePicture = dp.user?.profile_picture
          ? await this.minioService.generateImageUrl('client-images', dp.user?.profile_picture)
          : null;
        ...
        for (const doc of dp.DeliveryPersonDocuments) {
          const contratUrl =
            (await this.minioService.generateImageUrl('client-documents', doc.document_url)) || '';
          contracts.push({
    ...
    if (type === 'provider') {
      const [providers, total] = await this.providerRepository
        ...
        .where(
          q ? `(p.last_name ILIKE :q OR p.first_name ILIKE :q OR p.company_name ILIKE :q)` : '1=1',
          { q: `%${q}%` },
        )
        .skip((page - 1) * pageSize)
        .take(pageSize)
      ...
      for (const p of providers) {
        ...
        console.log(p.user);
        const profilePicture = p.user?.profile_picture
          ? await this.minioService.generateImageUrl('client-images', p.user?.profile_picture)
          : null;
    ...
    if (type === 'merchant') {
      const [merchants, total] = await this.merchantRepository
        ...
        .where(
          q ? `(m.last_name ILIKE :q OR m.first_name ILIKE :q OR m.company_name ILIKE :q)` : '1=1',
          { q: `%${q}%` },
        )
      ...
      for (const m of merchants) {
        ...
        const profilePicture = m.user?.profile_picture
          ? await this.minioService.generateImageUrl('client-images', m.user?.profile_picture)
          : null;
        const prenom = m.first_name ?? '';
        const contratUrl = m.contract_url
          ? await this.minioService.generateImageUrl('client-documents', m.contract_url)
          : '';
  ...
  async getVehicleCategories(): Promise<{ data: VehicleCategory[]; total: number }> {
    ...
    const vehicleCategories: VehicleCategory[] = categories.map((category) => ({
      id:
        category.category_id !== null && category.category_id !== undefined
          ? String(category.category_id)
          : null,
      name: category.name,
      max_weight: category.max_weight || 0,
      max_dimension:
        typeof category.max_dimension === 'string'
          ? parseFloat(category.max_dimension) || 0
          : category.max_dimension || 0,
    }));
    return {
