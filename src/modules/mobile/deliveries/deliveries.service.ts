...
export class DeliveriesService {
  ...
  async getDeliveryDetails(user_id: string, delivery_id: string): Promise<DeliveryDetails> {
    ...
    const deliveryPersonUser =
      delivery.delivery_person?.user.clients[0].first_name +
      ' ' +
      delivery.delivery_person?.user.clients[0].last_name;
    ...
    const deliveryDetails: DeliveryDetails = {
      ...
      deliveryPersonPhoto: deliveryPersonPhoto
        ? await this.minioService.generateImageUrl('client-images', deliveryPersonPhoto)
        : '',
  ...
  async takeDeliveryPackage(
    ...
    await this.mailer.sendMail({
      ...
      html: `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 20px auto; padding: 20px; border-radius: 8px; background-color: #f9f9f9;">
      <p>Bonjour,</p>
