...
export class DeliveryManService {
  ...
  async addVehicle(
    ...
    const category = await this.categoryRepository.findOne({
      where: { category_id: vehicleData.categoryId },
    });
    if (!category) {
    ...
    await this.minioService.uploadFileToBucket(
      'client-documents',
      imageFileName,
      vehicleData.image,
    );
    ...
    const documentUrl = await this.minioService.uploadFileToBucket(
      'client-documents',
      documentFileName,
  ...
  async getMyVehicles(
    ...
    page: number = 1,
    limit: number = 10,
  ): Promise<{ data: VehicleResponseDto[]; totalRows: number }> {
    const deliveryPerson = await this.deliveryPersonRepository.findOne({
      where: { user: { user_id: userId } },
    });
    if (!deliveryPerson) {
    ...
    const data = await Promise.all(
      vehicles.map(async (vehicle) => ({
        ...
        image: vehicle.image_url
          ? await this.minioService.generateImageUrl('client-documents', vehicle.image_url)
          : '',
        justification_file:
          vehicle.vehicleDocuments && vehicle.vehicleDocuments[0]
            ? await this.minioService.generateImageUrl(
                'client-documents',
                vehicle.vehicleDocuments[0].vehicle_document_url,
              )
            : '',
      })),
    );
  ...
  async isDeliveryPersonIsAdmissibleForThisDelivery(
    user_id: string,
    shipmentId: string,
  ): Promise<boolean> {
    ...
    for (const vehicle of vehicles) {
      ...
      const category = await this.categoryRepository.findOne({
        where: { category_id: vehicleCategory.category_id },
      });
      if (!category) {
