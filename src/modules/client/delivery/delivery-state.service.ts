...
export class DeliveryStateService {
  ...
  async takeDeliveryPackage(
    ...
    const invoiceDetails: InvoiceDetails = {
      ...
      lineItems: [{ label: 'Montant de la livraison', value: Number(delivery.amount) }],
    ...
    await this.mailer.sendMail({
      ...
      html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border-radius: 8px;">
                    <h2>Votre Facture de Livraison</h2>
                    <p>Bonjour,</p>
                    <p>Veuillez trouver ci-joint votre facture de livraison.</p>
                    <p>Merci de votre confiance,</p>
                    <p style="color: #555;">L'équipe Écodeli</p>
                </div>
                `,
      attachments: [
        {
    ...
    if (currentStep === 0 || currentStep === 1000) {
      ...
      await this.mailer.sendMail({
        ...
        html: `
                    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border-radius: 8px;">
                        ...
                        <p>Bonjour,</p>
                        <p>Le livreur vient de terminer la livraison du colis : <strong>${delivery.shipment.description}</strong>.</p>
                        <p>Veuillez lui fournir le code suivant pour valider la livraison :</p>
                        <div style="background-color: #f0f0f0; padding: 16px; border-radius: 6px; text-align: center; font-size: 20px; font-weight: bold;">
                        ${secretCode}
                        </div>
                        <p style="margin-top: 20px;">Merci de votre confiance,<br>L'équipe Écodeli</p>
                    </div>
                    `,
      });
      ...
      if (exchangePoint.warehouse_id) {
        await this.mailer.sendMail({
          ...
          html: `
                        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border-radius: 8px;">
                        <h2>Dépôt en Entrepôt</h2>
                        <p>Bonjour,</p>
                        <p>Voici le code de livraison pour le dépôt en entrepôt :</p>
                        <p><strong>${delivery.shipment.description}</strong></p>
                        <div style="background-color: #f0f0f0; padding: 16px; border-radius: 6px; text-align: center; font-size: 20px; font-weight: bold;">
                            ${secretCode}
                        </div>
        ...
        await this.mailer.sendMail({
          ...
          html: `
                        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border-radius: 8px;">
                        <h2>Dépôt en Boîte EcoDeli</h2>
                        <p>Bonjour,</p>
                        <p>Voici le code de livraison pour le dépôt dans une boîte EcoDeli :</p>
                        <p><strong>${delivery.shipment.description}</strong></p>
                        <div style="background-color: #f0f0f0; padding: 16px; border-radius: 6px; text-align: center; font-size: 20px; font-weight: bold;">
                            ${secretCode}
                        </div>
        ...
        const nextStepStore = delivery.shipment.stores.find(
          (store) => store.step === currentStep + 1,
        );
        if (nextStepStore) {
          ...
          if (nextDelivery && nextDelivery.delivery_person?.user?.email) {
            await this.mailer.sendMail({
              ...
              html: `
                            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border-radius: 8px;">
                            <h2>Collecte de Colis</h2>
                            <p>Bonjour,</p>
                            <p>Voici le code nécessaire pour récupérer le colis suivant :</p>
                            <p><strong>${delivery.shipment.description}</strong></p>
                            <div style="background-color: #f0f0f0; padding: 16px; border-radius: 6px; text-align: center; font-size: 20px; font-weight: bold;">
                                ${secretCode}
                            </div>
                            <p>Merci de votre collaboration,<br>L'équipe Écodeli</p>
                            </div>
                        `,
            });
          } else {
            await this.mailer.sendMail({
              ...
              html: `
                            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border-radius: 8px;">
                            <h2>Code de Livraison</h2>
                            <p>Bonjour,</p>
                            <p>Voici le code de validation pour la suite de la livraison :</p>
                            <p><strong>${delivery.shipment.description}</strong></p>
                            <div style="background-color: #f0f0f0; padding: 16px; border-radius: 6px; text-align: center; font-size: 20px; font-weight: bold;">
                                ${secretCode}
                            </div>
                            <p>Merci de votre confiance,<br>L'équipe Écodeli</p>
                            </div>
                        `,
            });
          }
  ...
  async validateDeliveryWithCode(
    ...
    await this.mailer.sendMail({
      ...
      html: `
                    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border-radius: 8px;">
                    <h2>Félicitations !</h2>
                    <p>Bonjour ${deliveryPerson.user?.clients[0]?.first_name || ''},</p>
                    <p>Votre livraison a été validée avec succès.</p>
                    <p>Vous venez de recevoir un montant de <strong>${promisedPrice} €</strong> sur votre compte.</p>
                    <p>Merci pour votre service et votre engagement !</p>
                    <p>Cordialement,<br>L'équipe Écodeli</p>
                    </div>
                `,
    });
    ...
    await this.mailer.sendMail({
      ...
      html: `
                    ...
                    <p>Nous vous confirmons que votre livraison a été validée avec succès par le livreur.</p>
                    <p>Merci de votre confiance !</p>
                    <p>Cordialement,<br>L'équipe Écodeli</p>
                    </div>
                `,
  ...
  async validateDelivery(deliveryId: string, user_id: string): Promise<{ message: string }> {
    ...
    await this.mailer.sendMail({
      ...
      html: `
                    ...
                    <h2 style="color: #2a9d8f;">Livraison Validée ✅</h2>
                    <p>Bonjour ${deliveryPerson.user?.clients[0]?.first_name || ''},</p>
                    <p>Votre livraison a été <strong>validée avec succès</strong>.</p>
                    <p>Vous avez reçu un montant de <strong>${promisedPrice} €</strong> sur votre compte.</p>
                    <p>Merci pour votre professionnalisme et votre engagement !</p>
