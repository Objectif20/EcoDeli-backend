...
export class DeliveryController {
  ...
  async createShipmentTrolley(
    ...
    const shipment = await this.shipmentService.createTrolleyShipment(
      createShipmentDTO,
      files,
      req.user.user_id,
    );
  ...
  @Post('step')
  async createShipmentStep(
    @Body() createDeliveryDto: CreateDeliveryDto,
    @Body('updatedAmount') updatedAmount: number,
  ) {
    const delivery = await this.deliveryService.createStepDelivery(
      createDeliveryDto,
      updatedAmount,
    );
  ...
  @UseGuards(ClientJwtGuard)
  async getCurrentPendingShipments(@Body('user_id') user_id: string) {
    return this.deliveryUtilsService.getMyCurrentShipmentsForNegoctation(user_id);
  }

  @Get('onGoingDeliveries')
  @UseGuards(ClientJwtGuard)
  async getOngoingDeliveries(@Body('user_id') user_id: string) {
    return this.deliveryService.getOngoingDeliveries(user_id);
  }

  @Get('subscriptionStat')
  @UseGuards(ClientJwtGuard)
  async getSubscriptionStat(@Body('user_id') user_id: string): Promise<SubscriptionForClient> {
    return this.deliveryUtilsService.getSubscriptionPlanForClient(user_id);
  }

  @Get('myCurrentShipments')
  @UseGuards(ClientJwtGuard)
  async getCurrentShipment(@Body('user_id') user_id: string): Promise<ShipmentListItem[]> {
  ...
  @UseGuards(ClientJwtGuard)
  async finishDelivery(@Param('id') deliveryId: string, @Body('user_id') user_id: string) {
    return this.deliveryStateService.finishDelivery(deliveryId, user_id);
  }

  @Post('delivery/:id/validate')
  @UseGuards(ClientJwtGuard)
  async validateDelivery(@Param('id') deliveryId: string, @Body('user_id') user_id: string) {
  ...
  @UseGuards(ClientJwtGuard)
  async cancelDelivery(@Param('id') deliveryId: string, @Body('user_id') user_id: string) {
  ...
  @UseGuards(ClientJwtGuard)
  async getMyLocation(@Body('user_id') user_id: string) {
    return this.deliveryService.getDeliveriesLocation(user_id);
  }

  @Get('delivery/current')
  @UseGuards(ClientJwtGuard)
  async getCurrentDeliveriesAsClient(@Body('user_id') user_id: string) {
  ...
  @Get('delivery/:id')
  @UseGuards(ClientJwtGuard)
  async getDeliveryById(@Param('id') delivery_id: string, @Body('user_id') user_id: string) {
  ...
  async bookShipment(
    ...
    return this.deliveryService.bookDelivery(shipment_id, user_id, startDate, endDate);
  }

  @Post(':id/bookPartial')
  @UseGuards(ClientJwtGuard)
  async bookPartial(@Param('id') id: string, @Body() bookPartialDTO: BookPartialDTO) {
    return this.deliveryService.bookPartial(bookPartialDTO, id);
  }

  @Post(':id/askNegociation')
  @UseGuards(ClientJwtGuard)
  async askNegociation(@Param('id') shipment_id: string, @Body('user_id') user_id: string) {
  ...
  @Get('office/:id')
  @UseGuards(ClientJwtGuard)
  async getOfficeById(@Param('id') shipment_id: string) {
    return this.shipmentService.getShipmentDetails(shipment_id);
  }

  @Get(':id')
  async getShipmentById(@Param('id') shipment_id: string) {
  ...
  @Delete(':id')
  async deleteShipment(@Param('id') shipmentId: string, @Body('user_id') user_id: string) {
