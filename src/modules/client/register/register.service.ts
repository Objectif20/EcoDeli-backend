...
export class RegisterService {
  ...
  async registerClient(clientDto: RegisterClientDTO): Promise<{ message: string }> {
    ...
    if (stripe_temp_key) {
      try {
        ...
        throw new BadRequestException(
          "Erreur lors de l'attachement du paymentMethod au client Stripe",
          error,
        );
      }
    }
    ...
    try {
      ...
      await this.mailer.sendMail({
        ...
        html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border-radius: 8px; background-color: #f0f8f5;">
        <h2 style="color: #2a9d8f;">Bienvenue chez Écodeli !</h2>
        <p>Bonjour,</p>
        <p>Pour finaliser la création de votre compte, merci de valider votre adresse email en cliquant sur le bouton ci-dessous :</p>
        <a href="${urlWithCode}" style="display: inline-block; padding: 12px 24px; background-color: #2a9d8f; color: white; text-decoration: none; border-radius: 6px; font-weight: bold;">
          Valider mon compte
        </a>
        <p style="margin-top: 20px; color: #555;">Si vous n’avez pas créé de compte chez nous, vous pouvez ignorer cet email.</p>
  ...
  async registerMerchant(merchantDto: RegisterMerchantDTO): Promise<{ message: string }> {
    ...
    if (stripe_temp_key) {
      try {
        ...
        console.log(error);
        throw new BadRequestException(
          "Erreur lors de l'attachement du paymentMethod au commerçant Stripe",
          error,
        );
      }
    }
  ...
  async createProvider(
    ...
    documentData: { name: string; provider_document_url: string }[],
    ...
    try {
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 20px auto; padding: 20px; border-radius: 8px; background: #f7f9f8;">
        <h2 style="color: #2a9d8f;">Validez votre compte Écodeli</h2>
        <p>Bonjour,</p>
        <p>Merci de cliquer sur le lien ci-dessous pour valider votre compte :</p>
        <p><a href="${urlWithCode}" style="color: #fff; background-color: #2a9d8f; padding: 12px 20px; border-radius: 6px; text-decoration: none; font-weight: bold;">Valider mon compte</a></p>
        <p style="color: #555;">Si vous n'avez pas demandé cette validation, vous pouvez ignorer ce message.</p>
        <p>Cordialement,<br>L'équipe Écodeli</p>
      </div>
    `,
      });
  ...
  async contractMerchant(merchant: any, imageBase64?: string): Promise<string> {
    ...
    doc
      .fontSize(14)
      .text(
        `Adresse: ${merchant.address}, ${merchant.postal_code} ${merchant.city}, ${merchant.country}`,
      );
    doc.moveDown();
    ...
    doc
      .fontSize(14)
      .text('Le marchand accepte les conditions générales d’utilisation de la plateforme.');
    doc.moveDown();
  ...
  async generateProviderContractPdf(provider: Providers, imageBase64?: string): Promise<string> {
    ...
    doc
      .fontSize(14)
      .text(
        `Adresse: ${provider.address}, ${provider.postal_code} ${provider.city}, ${provider.country}`,
      );
    doc.moveDown();
    ...
    doc
      .fontSize(14)
      .text(
        'Le prestataire accepte que ses données soient étudiées par EcoDeli afin de valider ou non son accès à la plateforme.',
      );
    doc.moveDown();
  ...
  async createDeliveryPerson(
    registerDeliveryPersonDto: RegisterDeliveryPersonDTO,
    deliveryPersonFiles: Array<Express.Multer.File>,
    user_id: string,
  ): Promise<{ message: string }> {
    const {
      license,
    ...
    const contractUrl = await this.generateDeliveryPersonContractPdf(
      savedDeliveryPerson,
      signature,
    );
  ...
  async generateDeliveryPersonContractPdf(
    ...
    const client = await this.clientRepository.findOne({
      where: { user: { user_id: deliveryPerson.user.user_id } },
    });
    ...
    doc
      .fontSize(14)
      .text(
        `Adresse: ${deliveryPerson.address}, ${deliveryPerson.postal_code} ${deliveryPerson.city}, ${deliveryPerson.country}`,
      );
    ...
    doc
      .fontSize(14)
      .text(
        'Le livreur accepte que ses données soient étudiées par EcoDeli afin de valider ou non son accès à la plateforme.',
      );
    doc.moveDown();
  ...
  async getLanguage(): Promise<
    { language_id: string; language_name: string; iso_code: string; active: boolean }[]
  > {
